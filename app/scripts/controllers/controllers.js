...
angular.module('sarusApp.controllers', [])
    .controller('PostsController', function($scope, $location, $http, $routeParams, $element, $timeout) {
        ...
        // As the client scrolls down, the sidebar have to updated with more slug
        // this function calls the slugs/:offset/:count api to get more slugs
        $scope.loadMoreSlugs = function() {
            var offset = $scope.titles[$scope.titles.length - 1].index + 1;
            $http.defaults.useXDomain = true;
            var httpRequest = $http({
                method: 'GET',
